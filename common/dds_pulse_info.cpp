...
unsigned
extract_flags(const std::string &s)
{
    unsigned flags = 0;
...
dds_pulse_info::dds_pulse_info()
    : iDDS(0), fOn(0), fOff(0), aOn(NO_AMP), aOff(0), sb(NO_SB)
{}

dds_pulse_info::dds_pulse_info(const std::string &s)
    : iDDS(0), fOn(0), fOff(0), aOn(NO_AMP), aOff(0), sb(NO_SB)
{
    updateFromString(s.c_str());
}
...
void dds_pulse_info::updateFromString(const char* s)
{
    if (strstr(s, "DDS") != 0) { //old format
        sscanf(s, "DDS(%u),%lf,%lf,%lf,%d,%X",
               &iDDS, &t, &fOn, &fOff, &sb, &flags);
    } else if (numOccurences(s, ",") == 5) {
        sscanf(s, "%u,%lf,%lf,%lf,%d,%X", &iDDS, &t, &fOn, &fOff, &sb, &flags);
    } else {
        sscanf(s, "%u,%lf,%lf,%lf,%u,%u,%d,%X",
               &iDDS, &t, &fOn, &fOff, &aOn, &aOff, &sb, &flags);
    }
}
...
int dds_pulse_info::to_string(char *s, size_t n) const
{
    ...
    // return snprintf(s, n, "%u,%d.%03u,%d.%09u,%d.%09u,%d,%X",
    //                 iDDS, us, ns, MHz[0], mHz[0], MHz[1], mHz[1], sb, flags);
    return snprintf(s, n, "%u,%d.%03u,%15.9f,%15.9f,%u,%u,%d,%X",
                    iDDS, us, ns, fOn, fOff, aOn, aOff, sb, flags);
}

bool operator==(const dds_pulse_info &a, const dds_pulse_info &b)
{
    return ((a.t == b.t) && (a.fOn == b.fOn) && (a.fOff == b.fOff) &&
            (a.aOn == b.aOn) && (a.aOff == b.aOff) && (a.sb == b.sb) &&
            (a.flags == b.flags));
}
...
dds_pulse3_info::dds_pulse3_info()
    : iDDS1(0), iDDS2(0), iDDS3(0), f1On(0), f2On(0), f3On(0), f1Off(0),
      f2Off(0), f3Off(0), sb(NO_SB)
{
}

dds_pulse3_info::dds_pulse3_info(const std::string &s)
    : iDDS1(0), iDDS2(0), iDDS3(0), f1On(0), f2On(0), f3On(0), f1Off(0),
      f2Off(0), f3Off(0), sb(NO_SB)
{
    updateFromString(s.c_str());
...
{
    sscanf(s, "%u,%u,%u,%lf,%lf,%lf,%lf,%lf,%lf,%lf,%d,%X", &iDDS1, &iDDS2,
           &iDDS3, &t, &f1On, &f2On, &f3On, &f1Off, &f2Off,
           &f3Off, &sb, &flags);
}
...
int dds_pulse3_info::to_string(char *s, size_t n) const
{
    ...
    return snprintf(s, n, "%u,%u,%u,%d.%03u,%15.9f,%15.9f,%15.9f,%15.9f,"
                    "%15.9f,%15.9f,%d,%X", iDDS1, iDDS2, iDDS3, us, ns, f1On,
                    f2On, f3On, f1Off, f2Off, f3Off, sb, flags);

}

bool operator==(const dds_pulse3_info &a, const dds_pulse3_info &b)
{
    return ((a.t == b.t) && (a.f1On == b.f1On) && (a.f2On == b.f2On) &&
            (a.f3On == b.f3On) && (a.f1Off == b.f1Off) &&
            (a.f2Off == b.f2Off) && (a.f3Off == b.f3Off) && (a.sb == b.sb) &&
            (a.flags == b.flags));
}
...
/******** Some shared functions for FPGA and PC programs that should
