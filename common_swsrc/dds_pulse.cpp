...
void
print_pulse_info(unsigned iDDS, unsigned ftwOn, unsigned ftwOff, unsigned t,
                 unsigned ttl, const char *info)
{
    double fClock = 1e9;
    ...
    if (info) {
        fprintf(gLog, "%s DDS(%u) fOn = %9u, fOff = %9u, "
                "t = %8.2f us TTL=%08X, (%s)\n", DDS_name(iDDS), iDDS,
                FTW2Hz(ftwOn, fClock), FTW2Hz(ftwOff, fClock),
